...
int verificaArestasParalelasOrientado (Grafo *g, int origem1, int destino1) {
    if (g->adjMatrix[origem1][destino1] >= 1 && g->adjMatrix[destino1][origem1] >= 1) {
        return 1;
    }
    return 0;
}
...
Grafo inverterGrafoOrientado (Grafo *g) {
    Grafo gInvertido;
    gInvertido.numVertices = g->numVertices;
    inicializarGrafo(&gInvertido, gInvertido.numVertices);
    for (int i = 0; i < gInvertido.numVertices; i++) {
        for (int j = 0; j < gInvertido.numVertices; j++) {
            if (g->adjMatrix[i][j] >= 1) {
                adicionarArestaOrientado(&gInvertido, j, i);
            }
        }
    }
    return gInvertido;
...
int* fechoTransitivoInverso (Grafo *g, int vertice) {
    Grafo gInvertido = inverterGrafoOrientado(g);
    int verticesVisitados[g->numVertices];
    for (int i = 0; i < g->numVertices; i++) {
        verticesVisitados[i] = 0;
    }
    dfs(&gInvertido, vertice, verticesVisitados);
    return verticesVisitados;
}